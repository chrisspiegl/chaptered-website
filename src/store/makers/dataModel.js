  ...
  return {
    ...
    mutations: {
      ...
      CLEANUP(state) {
        ...
        const unusedQueries = Object.values(state.cache.queries).filter((x) => !queriyIdsInUse.includes(x.id))
        const expiredQueries = unusedQueries.filter((x) => x.createdAt < Date.now() - maxCacheAge)
        ...
        const itemIdsInUse = Object.values(state.cache.queries).reduce((previous, queryDetail) => [...previous, ...asArray(queryDetail.response.data)], [])
